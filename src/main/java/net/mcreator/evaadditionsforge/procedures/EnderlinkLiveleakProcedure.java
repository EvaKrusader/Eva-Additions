...
public class EnderlinkLiveleakProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((entity instanceof ServerPlayer _plr0 && _plr0.level() instanceof ServerLevel
				&& _plr0.getAdvancements().getOrStartProgress(_plr0.server.getAdvancements().getAdvancement(new ResourceLocation("eva_additions:liveleak_death"))).isDone()) == false) {
			EvaAdditionsMod.queueServerWork((int) (Mth.nextInt(RandomSource.create(), 5, 10) * 20), () -> {
				...
				if (!world.isClientSide() && world.getServer() != null)
					world.getServer().getPlayerList().broadcastSystemMessage(Component.literal(
							((entity.getCapability(EvaAdditionsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new EvaAdditionsModVariables.PlayerVariables())).EnderlinkAssistant + "Be careful! You are more fragile than you may think!")),
							false);
			});
		}
	}
