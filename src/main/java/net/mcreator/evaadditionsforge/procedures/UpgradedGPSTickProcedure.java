...
public class UpgradedGPSTickProcedure {
		...
		if (entity.isInWaterOrBubble()) {
			if (EnchantmentHelper.getItemEnchantmentLevel(EvaAdditionsModEnchantments.WATER_PROOF.get(), itemstack) != 0) {
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal((("X : " + new java.text.DecimalFormat("##.##").format(Math.round(entity.getX()))) + " " + ("Y : " + new java.text.DecimalFormat("##.##").format(Math.round(entity.getY()))) + " "
							+ ("Z : " + new java.text.DecimalFormat("##.##").format(Math.round(entity.getZ()))))), true);
			} else {
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal((("X : " + new java.text.DecimalFormat("##.##").format(Math.round(entity.getX()) + Mth.nextInt(RandomSource.create(), -3, 3))) + " "
							+ ("Y : " + new java.text.DecimalFormat("##.##").format(Math.round(entity.getY()) + Mth.nextInt(RandomSource.create(), -3, 3))) + " "
