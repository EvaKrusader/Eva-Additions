...
public class WingGelItemTooltipProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity, ItemStack itemstack, List<Component> tooltip) {
		if (entity == null || tooltip == null)
			return;
		if (itemstack.getItem() == EvaAdditionsModItems.WING_GEL_ITEM.get()) {
			if (itemstack.getOrCreateTag().getDouble("gelPower") != 0) {
				...
				if (((entity.getCapability(EvaAdditionsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new EvaAdditionsModVariables.PlayerVariables())).aptitudeItem).getItem() == EvaAdditionsModItems.COGNITION.get()) {
					WingGelItemCognitionTooltipProcedure.execute(entity, itemstack, tooltip);
				}
			}
		} else if (itemstack.getItem() == EvaAdditionsModItems.GEL_PACKAGING.get()) {
			if (((entity.getCapability(EvaAdditionsModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new EvaAdditionsModVariables.PlayerVariables())).aptitudeItem).getItem() == EvaAdditionsModItems.COGNITION.get()) {
				GetItemRarityColorProcedure.execute(entity, itemstack);
				WingGelPackingCognitionTooltipProcedure.execute();
			}
		}
	}
}
